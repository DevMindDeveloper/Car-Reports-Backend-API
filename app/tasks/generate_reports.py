...
## task
@celery_app.task(name="save_car_records", base = Singleton)
def save_car_records():
    ...
    try:
        ...
        for record in received_records:
            ...
            db_records = session.query(Car).filter(Car.record_id == car_input['record_id']).first()
            if db_records:
                logger.info("Record already avaliable!")
            
            ## store in DB
            else:
                insert_car_record = Car(record_id = car_input['record_id'], date = car_input['today_date'], category = car_input['category'],
        ...
        for db_res in db_results:
            if db_res.record_id in received_records_ids:
                id_exist = True

            ## deletion
            elif not id_exist:
                logger.info("deleting record")
                delete_car_record = session.query(Car).filter(Car.record_id == db_res.record_id).first()            
            ...
            ## updation
                ...
                for res in received_records:
                    if res.get("objectId") == db_res.record_id:
                        car_record = res
                        break
